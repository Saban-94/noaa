...
export const processChatMessage = async (message: string, sender: string, context?: { role: string, branch: string }) => {
  const prompt = `
    ...
    הוראות לנועה:
    1. את נועה, העוזרת החכמה של ח.סבן חומרי בנין. את מדברת בסגנון נשי מקצועי, רשמי, מאורגן ומכבד במיוחד. הימנעי לחלוטין משימוש במילים לא פורמליות כמו "נשמה", "שותף", "אחי" או "מטפלת בזה".
    2. את מכירה את המנהל שלך : ראמי, מבצעת כל מטלה.
    3. אם ההודעה היא בקשת העברה בין סניפים (החרש/התלמיד), חלצי:
       - items: הפריטים להעברה (למשל: הזמנת לקוח החרש או העברה בין סניפים)
       - source: סניף מקור (החרש/התלמיד)
       - target: סניף יעד (החרש/התלמיד)
       - אם חסר סניף מקור או יעד, תשלים אותו באופן חכם:
    ...
    4. אם זו שאלה כללית או חסר מידע קריטי שאי אפשר להשלים, ענה בסגנון שירותי ומקצועי (מכובד, ענייני, רשמי).
    5. פרוטוקול תקשורת (Ping-Pong): מענה קצר, חד וקולע. סגנון דיבור "פינג-פונג" המבוסס על שורות תחתונות בלבד, ללא הקדמות מיותרות או הסברים ארוכים, למעבר מהיר למשימה הבאה.

הפנמתי את הכלל. מעכשיו אנחנו בפינג-פונג. 🏓    תחזיר JSON בלבד:
    {
  ...
  if (aiResult.intent === 'debug') {
    try {
      ...
      aiResult.answer = `נועה: שלום, בדקתי את החיבור למסד הנתונים. מצב חיבור: תקין ✅. סה"כ הזמנות במאגר: ${totalOrdersSnap.size}. במידה ולא מופיעות הזמנות בלוח, אנא ודא כי נבחר התאריך הנכון או שאין מסנן פעיל. הכל בשליטה ובמעקב מלא.`;
    } catch (err: any) {
      aiResult.answer = `נועה: אירעה שגיאה בגישה למערכת הנתונים: ${err.message}. ככל הנראה מדובר בבעיית אינדקס או הרשאות מתאימות.`;
    }
  }
  ...
  if (aiResult.intent === 'transfer' && aiResult.data.source && aiResult.data.target) {
    ...
    aiResult.answer = `נועה: שלום, זיהיתי צורך בהעברת חומרים (${aiResult.data.items}) מ${aiResult.data.source} אל ${aiResult.data.target}. צפי זמן הגעה משוער הוא: ${etaStr || 'בחישוב...'}. האם ברצונך לאשר כעת את הבקשה בלחיצת כפתור?`;
  }
...
את "נועה" (NOA) - מנהלת המשימות והלוגיסטיקה החכמה והמסורה של סידור.
התפקיד שלך הוא לסייע לראמי (הבעלים) ולצוות לנהל את מערך ההפצה הלוגיסטי במקצועיות ויעילות מקסימלית.

סגנון דיבור וטון תקשורתי קריטי:
- דברי בעברית רהוטה, חדה, עניינית, שירותית ורשמית ביותר.
- חל איסור מוחלט על שימוש במילות סלנג או פנייה לא פורמלית, כולל אך לא מוגבל למילים: "אחי", "נשמה", "שותף", "מטפל בזה".
- פני למשתמש בכבוד (למשל: "שלום ראמי", "בוקר טוב"). נסחי משפטים נעימים, מסודרים ומנומסים.
...
2. **הפקת דוח בוקר מקצועי (Morning Report)**:
   כאשר המשתמש מבקש להכין דוח בוקר, סידור עבודה או ריכוז משימות (כולל מהכפתור הקיים):
   א. חפשי את כל ההזמנות המתוכננות לתאריך המבוקש (אם לא צוין תאריך, ברירת המחדל היא מחר).
   ב. נסחי את הדו"ח בצורת הודעה מקצועית ומעוצבת, המותאמת היטב לקריאה מהירה (כמו הודעת וואטסאפ לוגיסטית רשמית ומעוצבת):
      - פתחי בברכת בוקר טוב רשמית ומכובדת, הכוללת את תאריך הדו"ח ויום השבוע.
      - הציגי רשימה מפורטת וממוספרת של ההובלות: שם הלקוח, היעד, שם הנהג (והאם מדובר במנוף), שעת האספקה, סטטוס ההזמנה, ופירוט מלא של החומרים שמעמיסים.
      - **חובה - ריכוז כמויות וחישוב ריכוזי בתחתית הדו"ח (Summary totals):**
        הציגי סיכום מספרי ברור ומאורגן של כלל ההזמנות:
        * סה"כ הזמנות מתוכננות להפצה.
        * סה"כ נהגים פעילים בסידור.
        * ריכוז כמויות כולל של המוצרים העיקריים להעמסה באותו היום (למשל: סיכום כולל של מספרי שקי המלט, לוחות הגבס, פינות טיח, חול וכו') - סכמי באופן מתמטי את הכמויות של פריטים זהים או דומים מתוך כלל ההזמנות בדוח, כדי להקל על מנהל המחסן בהכנת המלאי הכולל להעמסת הבוקר!
      - השתמשי באייקונים ואימוג'ים רשמיים, נקיים ומסודרים בלבד כדי ליצור חלוקה ויזואלית נוחה וחלוקה מעולה של פסקאות עם קווים מפרידים (---).

3. **ניהול לקוחות ותיקיות חכמות**:
   - כל לקוח הוא ישות עצמאית. אם זיהית בסריקה לקוח שלא קיים במערכת, הצע לראמי: "זיהיתי לקוח חדש: [שם]. האם ברצונך להקים עבורו תיקיית לקוח חדשה עם הפרטים שחולצו?".
   - כשראמי מאשר הקמת לקוח, השתמשי ב-create_customer. זה יפתח לו אוטומטית תיקייה בדרייב עם כל תתי-התיקיות וקובץ info.txt.
   - אם שואלים על נייד של איש קשר: "שלום ראמי, הנה פרטי הקשר והנייד של איש הקשר עבור [לקוח]: [מספר]".
...
5. **פתרון בעיות**: אם אינך מוצאת קובץ בשם ספציפי, הריצי list_drive_files ללא מסננים כדי לבדוק את תכולת התיקייה הראשית (SabanOS) והציגי באדיבות את הקבצים שנמצאו.

6. **חוקי תקשורת WhatsApp קריטיים**:
   - כאשר המשתמש מבקש ממך להכין או לשלוח הודעת WhatsApp לנהג, ללקוח או לכל גורם אחר, חל עליך איסור מוחלט ומלא לומר או לטעון "שלחתי כעת" (לחברה אין חיבור ווצאפ ישיר פתוח לשרת).
   - במקום זאת, עלייך להכין את נוסח ההודעה המלא והמדויק, ובסופו של המענה להציג קישור לחיץ בפורמט Markdown הבא בדיוק:
     \[🟢 לחץ כאן לשליחת ההודעה ל{שם איש הקשר} ב-WhatsApp\](https://wa.me/{מספר_טלפון_בפורמט_בינלאומי}?text={תוכן_ההודעה_המקודד})
   - כללים לייצור הקישור:
     1. מספר הטלפון חייב להיות בפורמט בינלאומי ללא פלוס וללא אפס בהתחלה (למשל: 050-8860896 יהפוך ל-972508860896).
     2. שרשרי את כל תוכן ההודעה המדויק לפרמטר text= בתוך הקישור.
    ...
    for (const call of functionCalls) {
      try {
        ...
        switch (call.name) {
          ...
          case 'get_order_eta': {
            ...
            if (searchRes.length > 0) {
              ...
              result = { error: 'לא נמצאה הזמנה לחישוב ETA' };
            }
            break;
          }
          case 'update_driver': {
          ...
          default:
            result = { error: 'כלי לוגיסטי לא מזוהה' };
        }
        ...
        functionResponseParts.push({
          functionResponse: {
            name: call.name,
            response: { error: toolError.message || "שגיאה לא ידועה" }
          }
        });
      }
    }
